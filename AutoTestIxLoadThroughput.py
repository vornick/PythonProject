...
def initialize_stat(ParsLogResultDir, num_of_platform):
    ...
    EndResultDir = ParsLogResultDir                                  #Блок инициализации переменных для функции парсинга статистики,
    tcp = 'TCP'
    udp='UDP'
    l2='L2'
    l3='L3'
...
def main(count_Of_Zero):                                        #Основная функция которая создает файл с результатом и
    if not os.path.exists(EndResultDir):                        #парсит ячейки в сгенеренных иксией файлах
...
def configfilecreator(RepPortIdClient, RepPortIdServer, RepCardIdClient, RepCardIdServer,
    ...
    CardIdClient = "?CardIdClient?"                                 #в начале функции есть список ключевых слов(LogName, CardIdClient и т.п. отмеченые ?), которые ищет функция
    ...
    FrameResult = "?FrameResult?"                                   # строка (из нее убираются кавычки и двойной слеш), так как в tcl файле строка с указанным путем имеет двойной слеш и кавычки
    ResultDir = '"?ResultDir?"'
